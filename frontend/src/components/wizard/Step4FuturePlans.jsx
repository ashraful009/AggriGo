...
import { FaBullseye, FaChartLine, FaLightbulb, FaGlobeAmericas, FaBullhorn, FaBoxOpen, FaCheckDouble, FaHandHoldingUsd, FaShip, FaChalkboardTeacher, FaArrowRight, FaArrowLeft, FaCheckCircle } from 'react-icons/fa';
...
const Step4FuturePlans = ({ onNext, onBack }) => {
  ...
  const [stepData, setStepData] = useState({
    ...
    supportNeeds: formData.supportNeeds || { marketing: false, packaging: false, quality: false, financing: false, exportSupport: false, training: false }
  });

  const [errors, setErrors] = useState({});

  const handleChange = (e) => { const { name, value, type, checked } = e.target; setStepData(prev => ({ ...prev, [name]: type === 'checkbox' ? checked : value })); };
  const handleSupportToggle = (key) => setStepData(prev => ({ ...prev, supportNeeds: { ...prev.supportNeeds, [key]: !prev.supportNeeds[key] } }));
  ...
  const textareaCls = "w-full px-4 py-3 rounded-xl border border-blue-100 focus:ring-2 focus:ring-blue-400 focus:border-transparent outline-none transition-all resize-none bg-white text-slate-700 placeholder-slate-400";
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 w-full max-w-5xl mx-auto font-sans text-slate-800">
      ...
      {/* Vision */}
      <div className="bg-white/80 backdrop-blur-sm p-8 rounded-2xl shadow-sm shadow-blue-100/60 border border-blue-100 space-y-6">
        <div>
          ...
          <textarea name="futureGoals" value={stepData.futureGoals} onChange={handleChange} className={`${textareaCls} ${errors.futureGoals ? 'border-red-400' : ''}`} rows="3" placeholder={t('form.step4.placeholders.goals') || "Describe your long-term vision..."} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <textarea name="productionIncreasePlan" value={stepData.productionIncreasePlan} onChange={handleChange} className={textareaCls} rows="3" placeholder={t('form.step4.placeholders.production') || "How do you plan to increase capacity?"} />
          </div>
          <div>
            <label className="flex items-center gap-2 text-sm font-bold text-slate-700 mb-3 uppercase tracking-wide">
            ...
            <textarea name="newProductsPlan" value={stepData.newProductsPlan} onChange={handleChange} className={textareaCls} rows="3" placeholder={t('form.step4.placeholders.newProducts') || "Any new product lines in mind?"} />
          </div>
        </div>
      </div>

      {/* Export Interest */}
      <div className="bg-gradient-to-r from-blue-50/60 to-indigo-50/40 p-8 rounded-2xl border border-blue-100">
        ...
        <div className="flex flex-wrap gap-4">
          {[t('form.step3.yes') || "Yes", t('form.step3.no') || "No"].map((option, index) => {
            ...
            return (
              <label key={option} className={`relative flex-1 min-w-[140px] flex items-center justify-center py-4 px-6 rounded-xl border-2 cursor-pointer transition-all duration-300 group
                  ${isSelected ? 'border-blue-400 bg-white shadow-md shadow-blue-100/50 text-blue-700 transform scale-[1.02]' : 'border-white bg-white/60 text-slate-500 hover:bg-white hover:border-blue-200'}`}>
                <input type="radio" name="interestInOnlineExport" value={val} checked={isSelected} onChange={handleChange} className="hidden" />
                <span className="font-bold text-lg">{option}</span>
                {isSelected && <div className="absolute top-3 right-3 text-blue-500"><FaCheckCircle /></div>}
              </label>
            );
          })}
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-5 px-1">
          ...
          <span className="text-xs font-bold text-blue-400 bg-blue-50 px-2 py-1 rounded border border-blue-100 uppercase tracking-wider">{t('form.step4.supportNote') || "Select all that apply"}</span>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {supportOptions.map((item) => {
            const isSelected = stepData.supportNeeds[item.key];
            return (
              <div key={item.key} onClick={() => handleSupportToggle(item.key)}
                className={`relative cursor-pointer p-5 rounded-xl border transition-all duration-200 flex items-start gap-4 select-none group
                  ${isSelected ? 'border-amber-300 bg-amber-50/80 shadow-sm ring-1 ring-amber-300' : 'border-blue-100 bg-white/80 hover:border-blue-300 hover:shadow-sm hover:bg-blue-50/20'}`}>
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl flex-shrink-0 transition-colors
                   ${isSelected ? 'bg-amber-400 text-amber-900' : 'bg-blue-50 text-blue-300 group-hover:text-blue-500 group-hover:bg-blue-100'}`}>
                  {item.icon}
                </div>
                <div>
                ...
                {isSelected && <div className="absolute top-3 right-3 text-amber-500"><FaCheckCircle /></div>}
              </div>
            );
          })}
        </div>
