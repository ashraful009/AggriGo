...
const TermsPage = () => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="absolute -top-20 -right-20 w-64 h-64 rounded-full blur-3xl opacity-20 bg-[#5BA4CF]" />
        <div className="relative z-10 max-w-2xl mx-auto">
          <div className="w-14 h-14 rounded-2xl bg-white/20 flex items-center justify-center mx-auto mb-4">
            <FaFileContract className="text-white text-2xl" />
          </div>
          <h1 className="text-3xl md:text-4xl font-extrabold mb-3">{t('nav.terms')}</h1>
          <p className="text-blue-100 text-sm md:text-base">{t('pages.terms.subtitle')}</p>
      ...
      <section className="py-12 px-4 flex-1">
        <div className="container mx-auto max-w-3xl">
          ...
          {SECTION_KEYS.map((key) => (
            <AccordionItem
              key={key}
              title={t(`pages.terms.sections.${key}_title`)}
              content={t(`pages.terms.sections.${key}_content`)}
